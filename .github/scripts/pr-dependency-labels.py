...
#
# Emphasis is allowed between the phrase and the number, because it is written
# both ways here: "**Stacked on #453**" closes after the number, "**Depends on**
# #428" closes before it. Requiring whitespace read the second as no
# declaration at all.
DECLARED = re.compile(
    r"\b(?:stacked on|depends on|blocked by|requires|on top of|after)[\s*_~`]+#(\d{2,5})\b",
    re.I,
)

# Code spans and fenced blocks are quotations, not declarations: a body that
# shows what another one says ("`**Depends on** #428` in #432") is citing a
# dependency, not having one. Read as prose it becomes the citing pull
# request's own -- which happened to this one.
QUOTED = re.compile(r"```.*?```|~~~.*?~~~|`[^`\n]*`", re.S)
...
def edges(prs):
    ...
    for n, p in prs.items():
        ...
        for m in DECLARED.findall(QUOTED.sub(" ", p["body"])):
            m = int(m)
            if m in prs and m != n:
                found.add((n, m))
    return found
