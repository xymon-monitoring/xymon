		       ...
		       FILE *output)
{
	 ...
	 *
	 * Only render graphs when explicitly requested (render_graphs=1).
	 * This allows xymond_filestore to generate HTML files without graphs,
	 * while the CGI can add them dynamically when someone views the page.
	 * Benefits:
	 * - Avoids RRD directory scanning on every status update
	 * - Graphs always reflect current RRD files (not stale)
	 * - Historical logs correctly omit graphs
	 */
	if (!is_history && render_graphs) {
		...
		// Check for manual GRAPHS specification first
		SBUF_MALLOC(graphs, 7 + strlen(service) + 1);
		snprintf(graphs, graphs_buflen, "GRAPHS_%s", service);
		graphsenv = getenv(graphs);

		if (graphsenv) {
			// Process manual graph specification
			fprintf(output, "<!-- GRAPHS_%s: %s -->\n", service, graphsenv);

			// For manual GRAPHS specification, we need to process each graph separately
			char *graphscopy = strdup(graphsenv);  // strtok modifies the string
			char *graphsptr = strtok(graphscopy, ",");
			...
			while (graphsptr != NULL) {
				...
				if (rrd_count > 0) {
					// Find the graph definition for this graph
					xymongraph_t *manual_graph = find_xymon_graph(graphsptr);
					if (manual_graph) {
						fprintf(output, "%s\n", xymon_graph_data(hostname, displayname, graphsptr, color, manual_graph, rrd_count, HG_WITHOUT_STALE_RRDS, HG_PLAIN_LINK, locatorbased, now-graphtime, now));
					} else {
						// If specific graph definition doesn't exist, try the original graph if RRD exists
						if (rrd && graph) {
							fprintf(output, "%s\n", xymon_graph_data(hostname, displayname, graphsptr, color, graph, rrd_count, HG_WITHOUT_STALE_RRDS, HG_PLAIN_LINK, locatorbased, now-graphtime, now));
						}
					}
				}
			...
			if (rrd_count > 0) {
				...
				fprintf(output, "<a name=\"begingraph\">&nbsp;</a>\n");
				fprintf(output, "%s\n", xymon_graph_data(hostname, displayname, service, color, graph, rrd_count, HG_WITHOUT_STALE_RRDS, HG_PLAIN_LINK, locatorbased, now-graphtime, now));
			}
		}
		else if (rrd && !graph) {
