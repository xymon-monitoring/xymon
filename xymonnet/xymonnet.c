...
/*
 * NTP tool selection and command construction. Kept in ntpcmd.c so
 * tests/network/ntp-cmd-harness.c can drive them directly (it #includes the
 * same file). NTPTOOL is a '|'-separated priority list of "ntpdate", "ntpdig",
 * "sntp" and "chronyd": the first installed tool wins, so a single name forces
 * that tool. The built-in default order is "ntpdig|ntpdate|sntp|chronyd" -
 * ntpdig first, because on ntpsec systems ntpdate is only a wrapper around
 * ntpdig. An explicit SNTP setting still forces sntp (backwards compatibility,
 * deprecated in favor of NTPTOOL="sntp").
 */
#include "ntpcmd.c"
