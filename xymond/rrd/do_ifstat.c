...
int do_ifstat_rrd(char *hostname, char *testname, char *classname, char *pagepaths, char *msg, time_t tstamp)
{
	...
	while (bol) {
		...
		switch (ostype) {
		  ...
		  default:
			/* Say so, as do_netstat and do_vmstat do for the same case:
			   otherwise an unresolved OS produces no ifstat data and no
			   clue why the graph stopped updating. */
			errprintf("Host '%s' reports ifstat for an unknown OS\n", hostname);
			break;
		}
