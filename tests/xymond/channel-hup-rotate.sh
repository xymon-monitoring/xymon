...
# Linux exposes fd/1 as a readlink-able symlink; NetBSD mounts procfs but its
# fd/ entries do not resolve through readlink, so require a real target here or
# fall through to the file-based check (the path FreeBSD, with no /proc, uses).
if [ -r "/proc/$worker/fd/1" ] && [ -n "$(readlink "/proc/$worker/fd/1" 2>/dev/null)" ]; then
	moved=no
	for _ in $(seq 1 100); do
