...
if ! grep -q "RRDtool probe failed" "$LOG"; then
	...
	# missing -- routine on a developer box or a minimal porting host, which is
	# where this test has to stay honest. If our stub never ran, the RRD path
	# was never exercised, so an early abort is an unmet host prerequisite, not
	# a #84 regression.
	#
