...
# Derive the repo root from the script's location, not from cwd, so the
# test works the same when invoked as ./tests/client/fs-filter.sh from
# anywhere -- including from a separate worktree.
