...
for cfg in $configs; do
	...
	while read -r task cmd; do
		...
		bin=$(basename "${cmd%% *}")
		src=$(find "$ROOT" -name "$bin.c" -not -path '*/tests/*' | head -1)
...
envfile=$(grep -oE 'ENVFILE[[:space:]]+\$XYMONCLIENTHOME/etc/[a-z.]+' "$ROOT/client/clientlaunch.cfg.DIST" | head -1 | awk '{print $2}')
